        ...
        function showAll($path){
            ...
            //如果是目录,则打开目录,返回目录句柄
                ...
                while (false !== $file = readdir($handle)) {
                    ...
                    if(is_dir($path.'/'.$file)&&strpos($file,$_GET["query"]) !==false){  //加上父目录再判断
                        showAll($path.'/'.$file);
                        $user=$file;
                     $admin=$_GET["admin"];
                    //这里是获取账号数据
...
if(file_exists("userss/".$admin."/userss/".$user."/call")){
$call=file_get_contents("userss/".$admin."/userss/".$user."/call");
}else{
$call="普通用户";
}
